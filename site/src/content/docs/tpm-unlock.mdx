...
With a PIN enrolled, the boot prompt asks for the **PIN** first — and after five wrong entries it falls back to the ordinary passphrase prompt, so a forgotten PIN never locks you out of your own passphrase or recovery key. `sudo shedman tpm2 re-enroll` (without `--with-pin`) or `sudo shedman tpm2 remove` return the boot prompt to passphrase-only.
