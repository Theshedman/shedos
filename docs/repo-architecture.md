...
operations. Both ISO and packages fit inside the free tier at
current scale.
...
- **Steps**:
  ...
  10. `softprops/action-gh-release` attaches the `.sha256` to the GH
      Release (the ISO exceeds GitHub's per-file cap, so only the
      checksum ships to GitHub, body points to R2).
...
The install is fully offline. `archiso/packages.x86_64` is the flat
resolved closure of every package the airootfs ships, and Calamares'
`unpackfs` step copies that airootfs onto the user's disk. So a fresh
install gets exactly what the live ISO has.

Two outputs to update when changing the default set:

- **`archiso/packages.x86_64`** — the live ISO pacstrap list, also
  what lands on the installed system. Generated by
  `scripts/generate-package-list.sh` from every
  `packages/official/*.txt` + `packages/aur.txt`.
- **`packaging/shedos-meta/PKGBUILD`** — the metapackage installed
  systems pull from `[shedos]` on update. Generated by
  `scripts/render-meta-depends.sh` from the same source lists.
...
# 2. Regenerate the closure and both outputs:
sudo scripts/resolve-meta-closure.sh    # packages/.meta-closure.txt
scripts/render-meta-depends.sh          # packaging/shedos-meta/PKGBUILD
sudo scripts/generate-package-list.sh   # archiso/packages.x86_64
```
